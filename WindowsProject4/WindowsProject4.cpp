...
{
    TCHAR str[100] = TEXT("Имя: Ваня Дондов");
    TCHAR str2[100] = TEXT("Работа: Продавец");
    TCHAR str3[100] = TEXT("Хобби: Спорт и чтение");
    int button;
    int result = 0;
    TCHAR res[25];
    switch (message)
    ...
    case WM_SHOWWINDOW:
    {
        button = MessageBox(hWnd, str, TEXT("Name"), MB_OK);
        MessageBox(hWnd, str2, TEXT("Job"), MB_OK);
        result = _tcslen(str) + _tcslen(str2) + _tcslen(str3);
        wsprintf(res, TEXT("Average: %d"), result);
        MessageBox(hWnd, str3, res, MB_OK);
    }
    case WM_COMMAND:
        {
